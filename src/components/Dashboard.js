...
import { fieldVisitRange, formatDateRangeEn, orderEventRange } from '../utils/dateRange';
import {
  buildSettlementRows,
...
export default function Dashboard() {
  ...
  const totalVisitDueNet = useMemo(() => {
    const t = todayISO();
    return netVisitPendingAfterStudioPay(orders, fieldVisits, (v, due) => due > 0 && isVisitFuture(v, t));
  }, [orders, fieldVisits]);
    ...
    <div className="panel">
      <div className="panel-head">
        <div>
          ...
          <p className="panel-lead">Dues, outside visits, and quick adds—at a glance.</p>
        </div>
      </div>

      <div className="quick-actions" role="group" aria-label="Quick actions">
        <button type="button" className="qa-btn" onClick={() => setTab('clients')}>
        ...
        <button type="button" className="qa-btn" onClick={() => setTab('orders')}>
          <span className="qa-emoji" aria-hidden="true">
            ✦
          </span>
          <span className="qa-text">
        ...
        <button type="button" className="qa-btn" onClick={() => setTab('field')}>
          <span className="qa-emoji" aria-hidden="true">
            ⌖
          </span>
          <span className="qa-text">
      ...
      {isFresh ? (
        <div className="welcome-banner">
          ...
          <p className="welcome-copy">
            Add a client and an order for studio jobs—or open <strong>My exposing</strong> for outside shoots (whose
            place, what to collect).
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            <button type="button" className="btn primary btn-lg shine" onClick={() => setTab('clients')}>
              Start with a client
            </button>
            <button type="button" className="btn btn-lg" onClick={() => setTab('field')}>
              Log my exposing
            </button>
          </div>
        </div>
      ...
      <div className="stats-row" role="list">
        <button type="button" className="stat-tile stat-tile-1" onClick={() => setTab('clients')}>
          <span className="stat-label">Clients</span>
          <span className="stat-value">{clients.length}</span>
          <span className="stat-hint">Tap to manage</span>
        </button>
        <button type="button" className="stat-tile stat-tile-2" onClick={() => setTab('orders')}>
          <span className="stat-label">Orders</span>
          <span className="stat-value">{orders.length}</span>
          <span className="stat-hint">Tap to open</span>
        </button>
        <button type="button" className="stat-tile stat-tile-3" onClick={() => setTab('field')}>
        ...
        <div className="stat-tile stat-tile-4 stat-tile-static">
          ...
          <span className="stat-value stat-value-money">{formatINR(totalClientDue)}</span>
          <span className="stat-hint">Orders not ended</span>
        </div>
        <button type="button" className="stat-tile stat-tile-6" onClick={() => setTab('field')}>
          <span className="stat-label">Pending (visits)</span>
          <span className="stat-value stat-value-money ok">{formatINR(totalVisitDueNet)}</span>
      ...
      <div className="dash-grid">
        <section className="card card-lift dash-card">
          <div className="dash-card-head">
            ...
            <p className="dash-card-subtitle">Upcoming jobs with balance left to receive.</p>
          </div>
          {clientDues.length === 0 ? (
            <div className="dash-empty dash-empty--orders">
              ...
              <p className="dash-empty-text">When a future job has money pending, it lands here.</p>
            ...
            <ul className="dash-feed">
              {clientDues.map(({ order, received, due }) => {
                ...
                return (
                  <li key={order.id} className="dash-feed-item dash-feed-item--client">
                    <div>
                    ...
                    <div className="dash-feed-money">
                      ...
                      <div className="dash-feed-received">Received {formatINR(received)}</div>
                    </div>
                  </li>
                );
              })}
        ...
        <section className="card card-lift dash-card">
          <div className="dash-card-head">
            ...
            <p className="dash-card-subtitle">Upcoming shoots where you still need to collect.</p>
          </div>
          {dashboardVisitRows.length === 0 ? (
            <div className="dash-empty dash-empty--visits">
              ...
              <p className="dash-empty-text">Log my exposing with an amount to see it here.</p>
            ...
            <>
              <ul className="dash-feed">
                {dashboardVisitRows.map(({ v, due }) => {
                  ...
                  return (
                    <li key={v.id} className="dash-feed-item dash-feed-item--visit">
                      <div className="dash-visit-when">
                        <span className="dash-pill-date" title={isRange ? 'From – to' : 'Single day'}>
                          {rangeLabel}
                        </span>
                        {v.time ? <span className="dash-visit-time muted">{v.time}</span> : null}
                      </div>
                      <div className="dash-visit-host">{v.hostName}</div>
                      {v.venue ? <div className="dash-visit-venue muted">{v.venue}</div> : null}
                      <div className="dash-visit-due">
                        <span className="muted">Still due</span>
                        <span className={due > 0 ? 'dash-visit-due-amt warn' : 'dash-visit-due-amt ok'}>
        ...
        <section className="card card-lift dash-card card-wide dash-card--settle">
          <div className="dash-card-head">
            ...
            <p className="dash-card-subtitle">
              Field <strong>due</strong> minus studio <strong>Pay them</strong> when it&apos;s the same contact
              (name or Match key).
            </p>
          </div>
          {settlementHint ? (
            <p className="dash-settle-hint">
              Studio pay and visit money both exist, but nothing linked—use the same <strong>Match key</strong> on the
              guest and the visit (or similar names).
            </p>
          ) : null}
              ...
              <table className="settlement-table">
                ...
                <tbody>
                  {settlementRows.map((r) => (
                    <tr key={r.key} className={r.hasBothSides ? 'settlement-row--net' : ''}>
                      <td>{r.label}</td>
                      <td>{r.payToGuest > 0 ? formatINR(r.payToGuest) : '—'}</td>
                      <td>{r.collectDue > 0 ? formatINR(r.collectDue) : '—'}</td>
                      <td>
